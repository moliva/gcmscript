...
/**
 * A GCM procedure to implement the <code>remote-load()</code> action which loads procedure definitions from a file
 * containing source code, and make them available for later invocation by name in the engine of the
 * {@link PAReconfigurationController reconfiguration controller} of a GCM component.
 *
 * @author The ProActive Team
 */
public class RemoteLoadAction extends AbstractGCMProcedure implements NativeProcedure, BindingController {
    /**
    ...
    public Signature getSignature() {
        return new Signature(OBJECT, model.getNodeKind("component"), STRING);
    }

    /**
     * Loads procedure definitions from the source code contained in the file of the given name, and make them
     * available for later invocation by name in the engine of the
     ...
     * and as second element the name of the file containing the source code of the procedure definitions to load.
    ...
    public Object apply(List<Object> args, Context ctx) throws ScriptExecutionError {
        if (args.get(0) instanceof GCMComponentNode) {
            try {
                ...
                return rc.load((String) args.get(1));
            } catch (NoSuchInterfaceException nsie) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to load procedure definitions remotely"), nsie);
            } catch (ReconfigurationException re) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to load procedure definitions remotely"), re);
            }
        }
        return null;
    }
