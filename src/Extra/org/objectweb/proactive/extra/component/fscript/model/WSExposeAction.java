...
/**
 * A GCM procedure to implement the <code>ws-expose()</code> action, which exposes as web services one, several or
 * all server interfaces of a GCM component.
 *
 * @author The ProActive Team
 */
public class WSExposeAction extends AbstractGCMProcedure {
    /**
    ...
    /**
     * Exposes as web services one, several or all server interfaces of a GCM component.
     *
     * @param args The arguments of the procedure call. Must contain as first element a {@link GCMInterfaceNode}
     * representing a server interface if only this server interface must be exposed as web service. Must contain as
     * first element a set of {@link GCMInterfaceNode} representing a set server interfaces if several server
     * interfaces must be exposed as web services. Must contain as first element a {@link GCMComponentNode}
     * representing a GCM component if all the server interfaces of this GCM component must be exposed as web
     * services.
    ...
    public Object apply(List<Object> args, Context ctx) throws ScriptExecutionError {
        if ((args.get(0) instanceof GCMInterfaceNode) || (args.get(0) instanceof Set<?>) ||
            ...
            try {
                            ...
                            throw new ScriptExecutionError(
                                Diagnostic
                                        .error(
                                                ...
                                                "Unable to expose component interfaces as web services: given server interfaces do not belong to the same GCM component"));
                        }
                    }
                } else {
                ...
                wsc.initServlet();
                wsc.exposeComponentAsWebService(GCM.getNameController(comp).getFcName(), itfNames);
            } catch (NoSuchInterfaceException nsie) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to expose component interface(s) as web service(s)"), nsie);
            } catch (UnknownFrameWorkException ufe) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to expose component interface(s) as web service(s)"), ufe);
            } catch (ActiveObjectCreationException aoce) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to expose component interface(s) as web service(s)"), aoce);
            } catch (NodeException ne) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to expose component interface(s) as web service(s)"), ne);
            } catch (WebServicesException wse) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to expose component interface(s) as web service(s)"), wse);
            }
        }
        return null;
    }
