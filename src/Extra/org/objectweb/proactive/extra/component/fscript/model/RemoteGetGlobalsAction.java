...
/**
 * A GCM procedure to implement the <code>remote-get-globals()</code> action which returns the names of all the
 * currently defined global variables in the engine of the
 * {@link PAReconfigurationController reconfiguration controller} of a GCM component.
 *
 * @author The ProActive Team
 */
public class RemoteGetGlobalsAction extends AbstractGCMProcedure implements NativeProcedure,
        BindingController {
    /**
    ...
    public Signature getSignature() {
        return new Signature(OBJECT, model.getNodeKind("component"));
    }

    /**
     * Returns the names of all the currently defined global variables in the engine of the
    ...
    public Object apply(List<Object> args, Context ctx) throws ScriptExecutionError {
        if (args.get(0) instanceof GCMComponentNode) {
            try {
                ...
                return rc.getGlobals();
            } catch (NoSuchInterfaceException nsie) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to get names of all global variables remotely"), nsie);
            } catch (ReconfigurationException re) {
                throw new ScriptExecutionError(Diagnostic.error(SourceLocation.UNKNOWN,
                        "Unable to get names of all global variables remotely"), re);
            }
        }
        return null;
    }
